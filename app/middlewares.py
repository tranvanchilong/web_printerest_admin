...
middleware = [
    ...
    Middleware(BrotliMiddleware, quality=4, mode="generic", lgwin=22, lgblock=0, minimum_size=400, gzip_fallback=True),
    Middleware(SessionMiddleware, secret_key=SESSION_SECRET_KEY, max_age=SESSION_MAX_AGE),
    Middleware(DatabaseMiddleware), # add db to request.state using middleware
